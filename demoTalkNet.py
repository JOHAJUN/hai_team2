		...
		for duration in durationSet:
			...
			with torch.no_grad():
				for i in range(batchSize):
					inputA = torch.FloatTensor(audioFeature[i * duration * 100:(i+1) * duration * 100,:]).unsqueeze(0).to('cpu')
					inputV = torch.FloatTensor(videoFeature[i * duration * 25: (i+1) * duration * 25,:,:]).unsqueeze(0).to('cpu')
...
def visualization(tracks, scores, args):
    ...
    for tidx, track in enumerate(tracks):
        ...
        for fidx, frame in enumerate(track['track']['frame'].tolist()):
            s = numpy.mean(score[max(fidx - 2, 0): min(fidx + 3, len(score))])
            faces[frame].append({
                'track': tidx,
                'score': float(s),
    ...
    crop_w = 720
    crop_h = 1280
    out = cv2.VideoWriter(os.path.join(args.pyaviPath, 'video_out.avi'),
                          cv2.VideoWriter_fourcc(*'XVID'), 25, (crop_w, crop_h))

    last_speaker_id = None
    cx, cy = fw // 2, fh // 2  # 초기 중심

    for idx, fname in tqdm.tqdm(enumerate(flist), total=len(flist)):
        frame = cv2.imread(fname)
        speaking_faces = [f for f in faces[idx] if f['score'] >= 0]

        if speaking_faces:
            current_id = speaking_faces[0]['track']
            if current_id != last_speaker_id:
                last_speaker_id = current_id
                cx = int(speaking_faces[0]['x'])
                cy = int(speaking_faces[0]['y'])
        ...
        cropped = numpy.zeros((crop_h, crop_w, 3), dtype=numpy.uint8)
        frame_crop = frame[y1:y2, x1:x2]
        if frame_crop.shape[0] != crop_h or frame_crop.shape[1] != crop_w:
    ...
    final_path = os.path.join(args.pyaviPath, 'video_out_final.avi')
    cmd = f"ffmpeg -y -i {os.path.join(args.pyaviPath, 'video_out.avi')} -i {audio_path} -c:v copy -c:a aac -shortest {final_path} -loglevel quiet"
    os.system(cmd)
